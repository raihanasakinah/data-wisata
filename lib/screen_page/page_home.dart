import 'package:data_wisata/screen_page/update.dart';
...
class _PageWisataState extends State<PageWisata> {
  ...
  Future<void> fetchWisata() async {
    final response = await http.get(Uri.parse('http://192.168.100.128/data_wisata/getWisata.php'));
  ...
  Future<void> deleteWisata(String id) async {
    final response = await http.post(
      Uri.parse('http://192.168.100.128/data_wisata/deleteWisata.php'),
      body: {'id': id},
    );
          ...
          return Card(
            ...
            child: ListTile(
              leading: wisata.gambar.isNotEmpty
                  ? Image.network(
                'http://192.168.100.128/data_wisata/gambar//${wisata.gambar}',
                width: 50,
                height: 50,
                fit: BoxFit.cover,
              )
